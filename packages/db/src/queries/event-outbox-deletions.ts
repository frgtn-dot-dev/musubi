...
export async function retainUnmappedEventDeletion(
  ...
  if (!candidates.length) {
    ...
    const [completed] = await tx.select({ id: eventOutbox.id }).from(eventOutbox).where(and(
      ...
      eq(eventOutbox.externalCalendarLinkID, target.id), eq(eventOutbox.status, "completed"), sql`((${eventOutbox.action} = 'delete' and ${eventOutbox.payload}->'caldavSeriesDeletion' is not null) or (${eventOutbox.action} = 'update' and (${eventOutbox.payload}->'caldavSeries'->'write'->'followingDelete' is not null or ${eventOutbox.payload}->'caldavSplit' is not null)))`,
      sql`coalesce(${eventOutbox.payload}->'caldavSeriesDeletion'->'context'->'mappings', ${eventOutbox.payload}->'caldavSeries'->'context'->'mappings', ${eventOutbox.payload}->'caldavSplit'->'prepared'->'context'->'mappings') @> ${JSON.stringify([{ externalEventID }])}::jsonb`,
    )).limit(1);
    if (completed) return;
  }
  await tx
