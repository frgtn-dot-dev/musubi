...
async function main() {
  ...
  try {
    ...
    await db.insert(account).values({ id: accountRowID, accountId: accountID, providerId: "google", userId: userID });
    await db.insert(externalCalendars).values({ id: sourceID, provider: "google", userID, accountID, calendarID: calendar.id, externalCalendarID: "private-remote", providerAccessRole: "owner" });
    ...
    await db.insert(eventOutbox).values({ id: receiptID, actorID: userID, mutationID: randomUUID(), position: 0, eventID: event.id, revision: 1, calendarID: calendar.id, externalCalendarLinkID: sourceID, provider: "google", userID, accountID, externalCalendarID: "private-remote", action: "delete", payload: { event }, status: "cancelled" });
    ...
    for (const role of ["reader", "writerWithoutPrivateAccess", "unknown", "unexpected"]) {
    ...
    await source({ providerAccessRole: "writer", providerAccessRevision: 2, cursor: null });
    ...
    await source({ providerAccessRole: "owner" });
    for (const role of ["viewer", "editor", "owner"]) {
      ...
      await expectTitle(role === "viewer" ? "Calendar event" : event.title, `local ${role}`);
    }
    ...
    await db.update(eventOutbox).set({ provider: "microsoft" }).where(eq(eventOutbox.id, receiptID));
    await expectTitle(event.title, "other providers preserve existing behavior");
    await db.update(eventOutbox).set({ provider: "google" }).where(eq(eventOutbox.id, receiptID));
    ...
    await db.insert(externalCalendars).values({ provider: "google", userID, accountID, calendarID: calendar.id, externalCalendarID: "private-remote", providerAccessRole: "owner" });
    await db.insert(account).values({ id: accountRowID, accountId: accountID, providerId: "google", userId: userID });
  ...
  console.log("Google delivery inbox privacy integration: OK");
}
