...
import {
  ...
  ProviderReminderEditSchema, providerReminderDesiredState, type ProviderRsvpInstance, type ProviderReminderEdit, type ProviderReminderInstanceIntent, type Event,
} from "@musubi/types";
import { db } from "..";
import {
  calendarEvents,
...
export type ProviderReminderInstanceContext = { actorID: string; eventID: string; request: ProviderReminderEdit; event: Event; mappingID: string; externalEventID: string; etag: string; accountID: string; externalCalendarID: string; linkID: string; state: import("@musubi/types").ProviderEventState; instance: ProviderRsvpInstance };
export type ProviderReminderInstanceReceipt = { operationID: string; replayed: boolean; status: string };
export type ProviderReminderInstancePreparation = { kind: "replay"; receipt: ProviderReminderInstanceReceipt } | { kind: "prepared"; context: ProviderReminderInstanceContext };
export async function prepareProviderReminderInstanceEdit(actorID: string, eventID: string, input: unknown): Promise<ProviderReminderInstancePreparation> {
  return reminderInstanceTransaction(actorID, eventID, input);
}
export async function commitProviderReminderInstanceEdit(prepared: ProviderReminderInstanceContext, baseline: Record<string, unknown>, nativeTime: EventTimeModel): Promise<ProviderReminderInstanceReceipt> {
  const frozen = structuredClone({ context: prepared, baseline, nativeTime: EventTimeModelSchema.parse(nativeTime) });
  const result = await reminderInstanceTransaction(frozen.context.actorID, frozen.context.eventID, frozen.context.request, frozen);
  if (result.kind !== "replay") throw new Error("instance reminder commit did not produce a receipt");
 ...
 * Private prepare/commit: provider preflight runs outside this transaction. */
async function reminderInstanceTransaction(
  actorID: string,
  eventID: string,
  input: unknown,
  prepared?: { context: ProviderReminderInstanceContext; baseline: Record<string, unknown>; nativeTime: EventTimeModel },
): Promise<ProviderReminderInstancePreparation> {
  ...
  return db.transaction(async (tx) => {
    ...
    if (previous) {
      ...
      if (
        ...
        JSON.stringify(
          previous.payload.reminderInstance?.request &&
            ProviderReminderEditSchema.parse(previous.payload.reminderInstance?.request),
    ...
    if (!instance) throw new EventWriteError("event-write", "unsupported");
    if (
    ...
    if (pending.length)
      throw new BadRequestError(
        "This source has a pending operation. Reconcile it before changing provider instance reminder.",
      );
    ...
    if (latest?.status === "cancelled")
      throw new BadRequestError(
        "The previous source operation was cancelled. Reconcile delivery before changing provider instance reminder.",
      );
    ...
    const desiredState = providerReminderDesiredState(state, request.reminders);
    const context: ProviderReminderInstanceContext = { actorID, eventID, request, event: snapshot, mappingID: mapping.id, externalEventID: mapping.externalEventID, etag: mapping.etag, accountID: target.link.accountID, externalCalendarID: target.link.externalCalendarID, linkID: target.link.id, state, instance };
    if (!prepared) return { kind: "prepared", context };
    if (!isDeepStrictEqual(context, prepared.context) || !isDeepStrictEqual(context.event.timeModel, prepared.nativeTime)) throw new BadRequestError("instance reminder source changed during provider verification.");
    ...
    if (prepared.baseline.id !== mapping.externalEventID || prepared.baseline.etag !== mapping.etag)
      throw new BadRequestError("instance reminder provider identity changed.");
    const reminderInstance: ProviderReminderInstanceIntent = { request, baseline: prepared.baseline, nativeTime: prepared.nativeTime, instance, baselineState: state, desiredState, mappingID: mapping.id };
    const id = randomUUID();
    await appendEventOutbox(tx, snapshot, [
      {
        ...
        action: "update",
        payload: { event: snapshot, reminderInstance },
      },
    ]);
...
export async function hasProviderReminderInstanceSource(row: import("./event-outbox").EventOutboxRow): Promise<boolean> {
