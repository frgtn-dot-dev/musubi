...
export async function purgeDeletedEvents(before: Date) {
	await db
      ...
      where ${eventOutbox.eventID} = coalesce(${events.seriesID}, ${events.id})
        and (${eventOutbox.payload}->'caldavSplit' is not null or ${eventOutbox.payload}->'caldavSeriesDeletion' is not null or ${eventOutbox.payload}->'caldavSeries' is not null)
