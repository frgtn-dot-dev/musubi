...
export function providerEventDetails(state: ProviderEventState) {
  ...
  if (reminder.provider === "google") {
    add("Provider reminders", reminder.useDefault === true ? "Calendar defaults" : reminder.useDefault === null ? "Not reported" : reminder.overrides.length ? reminder.overrides.map(item => `${item.method ?? "Unknown method"} · ${item.minutes === null ? "time not reported" : `${item.minutes} minutes before start`}`).join("\n") : "Off");
  } else if (reminder.provider === "microsoft") {
    add("Provider reminders", reminder.isOn === false ? "Off" : reminder.isOn === null ? "Not reported" : reminder.minutesBeforeStart === null ? "On · time not reported" : `${reminder.minutesBeforeStart} minutes before start`);
  } else {
    add("Provider alarms", reminder.alarms.length ? reminder.alarms.map(alarm => [alarm.action ?? "Unknown action", alarm.trigger ?? "trigger not reported", alarm.related, alarm.repeat ? `repeat ${alarm.repeat}` : null, alarm.duration].filter(Boolean).join(" · ")).join("\n") : "None in this event");
  }
