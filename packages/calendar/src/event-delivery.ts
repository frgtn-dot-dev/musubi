...
export function eventDeliveryExplanation(target: EventDeliveryTarget): string {
  ...
  if (target.organizerPhase === "queued" && target.status === "cancelled") return "Musubi stopped before sending this meeting action. Sync and reopen the current meeting before trying again.";
