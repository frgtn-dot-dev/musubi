import { z } from "zod";
import { CivilDateTimeSchema, EventTimeZoneSchema } from "./event_time";
...
export const ProviderEventStateResponseSchema = z.object({ state: ProviderEventStateSchema.nullable(), version: z.string().regex(/^[0-9a-f]{64}$/).nullable().optional(), reminderEdit: z.discriminatedUnion("provider", [z.object({ provider: z.literal("google"), expectedRevision: z.number().int().positive() }).strict(), z.object({ provider: z.literal("caldav"), expectedRevision: z.number().int().positive(), minutesBeforeStart: z.number().int().min(0).max(40320).nullable(), scope: z.literal("series").optional() }).strict()]).optional(), outlookOccurrenceMove: z.object({ calendarID: z.uuid() }).strict().optional(), outlookSeriesContent: z.object({ time: z.object({ kind: z.literal("zoned"), timeZone: EventTimeZoneSchema, startLocal: CivilDateTimeSchema, endLocal: CivilDateTimeSchema }).strict().optional(), calendarID: z.uuid(), expectedRevision: z.number().int().positive(), seriesVersion: z.string().regex(/^[0-9a-f]{64}$/), content: z.object({ title: z.string(), description: z.string().nullable(), location: z.string().nullable() }).strict() }).strict().optional(), outlookCancellation: z.object({ calendarID: z.uuid(), expectedRevision: z.number().int().positive(), seriesVersion: z.string().regex(/^[0-9a-f]{64}$/), scopes: z.array(z.enum(["occurrence", "series"])).min(1) }).strict().optional(), organizerEdit: z.object({ seriesVersion: z.string().regex(/^[0-9a-f]{64}$/).optional(), scope: z.enum(["occurrence", "series"]).optional(), instanceVersion: z.string().regex(/^[0-9a-f]{64}$/).optional(), provider: z.enum(["google", "caldav", "microsoft"]), calendarID: z.uuid(), expectedRevision: z.number().int().positive(), actions: z.array(z.enum(["update", "delete"])).min(1).optional(), timeEdit: z.literal(true).optional(), timeZone: EventTimeZoneSchema.optional(), timeKind: z.literal("all-day").optional() }).strict().optional(), rsvpEdit: z.object({ provider: z.enum(["google", "caldav", "microsoft"]), expectedRevision: z.number().int().positive(), scope: z.enum(["occurrence", "series"]).optional(), series: z.object({ version: z.string().regex(/^[0-9a-f]{64}$/), ownResponse: z.string() }).strict().optional() }).strict().optional() }).strict();
