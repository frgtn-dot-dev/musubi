...
export const ProviderEventStateResponseSchema = z.object({ state: ProviderEventStateSchema.nullable(), version: z.string().regex(/^[0-9a-f]{64}$/).nullable().optional(), reminderEdit: z.discriminatedUnion("provider", [z.object({ provider: z.literal("google"), expectedRevision: z.number().int().positive() }).strict(), z.object({ provider: z.literal("caldav"), expectedRevision: z.number().int().positive(), minutesBeforeStart: z.number().int().min(0).max(40320).nullable(), scope: z.literal("series").optional() }).strict()]).optional(), organizerEdit: z.object({ provider: z.literal("google"), calendarID: z.uuid(), expectedRevision: z.number().int().positive() }).strict().optional(), rsvpEdit: z.object({ provider: z.enum(["google", "caldav", "microsoft"]), expectedRevision: z.number().int().positive() }).strict().optional() }).strict();
