...
```
          ┌───────────┐        calendar_events        ┌────────────┐
          │  events   │◄──────── (many-to-many) ──────►│ calendars  │
          └───────────┘                                └────────────┘
                │                                              │
                │ originCalendarID  ── the "home" calendar ────┘
                │    (governs who may edit the event)
```
...
```
apps/client ─┐
             ├──► packages/types      (shared Zod contract — both apps speak this)
apps/api ────┤──► packages/calendar   (recurrence logic — both apps expand RRULEs)
             │
apps/api ────┼──► packages/db         (server only — the DB is never touched by the client directly)
             ├──► packages/auth
             └──► packages/config
```
...
```
 ┌─────────── apps/client ───────────┐         ┌──────────────── apps/api ────────────────┐
 │                                    │         │                                           │
 │  AddEventModal                     │  HTTPS  │  index.ts route                           │
 │    → useEventsStore.addEvent()     │ ──────► │    → requireAuth (middleware)             │
 │      → useApi().createEvent()      │         │      → handlerCreateEvent                 │
 │                                    │         │        → assertCan(user, cal,'editEvents')│
 │                                    │         │        → createEvent()  [packages/db]     │
 │  optimistic-ish: store + cache     │         │        → pushEventToProviders() [sync]    │
 │  written after the call succeeds   │ ◄────── │        → notifyCalendarMembers() [SSE]    │
 │                                    │  JSON   │        → 201 + event JSON                 │
 │  local SQLite cache (offline)      │         │                                           │
 └────────────────────────────────────┘         └───────────────────────────────────────────┘
```
