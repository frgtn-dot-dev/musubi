...
This section is the contributor's map of the codebase. Read this page first — it gives you the mental models that the rest of the code assumes you already have. Then jump to the part you're working on. (Brand new to the project? The [Codebase Onboarding](/docs/guides/onboarding/) course walks you through everything in order, hands-on.)
...
<CardGrid>
  ...
  <LinkCard title="Federation" href="/docs/architecture/federation/" description="Musubi ↔ Musubi — cross-server calendar sharing." />
  ...
  <LinkCard title="Glossary" href="/docs/architecture/glossary/" description="Drill, dock, ghost, shadow account… the codebase's vocabulary." />
</CardGrid>
...
sequenceDiagram
    ...
    H->>P: prepareEventWrites(intent, identity) / preflight
    H->>DB: createEvent(event, calendars, deliver.outbox)
    Note over DB: local event + outbound intent commit together
    Note over H: notifyCalendarMembers() broadcast (SSE)
    H->>P: deliver() / claim persisted operation
    H-->>API: 201 + event JSON
    API-->>S: write store + local SQLite cache
```

Most writes cover the same five concerns on the server — **validate → authorise
→ preflight → commit event + outbox → broadcast/deliver**. Provider-backed calendar writes may
order provider and database work differently; the exact failure semantics are
in the [API guide](/docs/architecture/api/) and
[sync guide](/docs/architecture/sync/#write-path-and-failure-semantics).
