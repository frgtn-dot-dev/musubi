...
<Aside type="note">
Broadcasts are **best-effort and fire-and-forget** — a member with no open connection (offline, backgrounded) simply catches up on their next pull-to-refresh delta sync (`useRefreshData`). There is no auto-reconnect loop and no server-side replay buffer; the delta sync is the durable path, SSE is the live-nicety on top.
</Aside>
