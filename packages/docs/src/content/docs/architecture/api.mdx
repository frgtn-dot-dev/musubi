...
```
1. cors()                    whitelist config.api.url + dev localhosts; credentials on
2. express.json({512kb})     512 KB cap — avatars arrive as base64 JSON
3. middlewareLogHandler      logs method/path/status
4. app.all("/api/auth/*")    Better Auth owns sign-in/up, sessions, password reset
5. app.<verb>("/api/v1/…")   the application routes (see below)
6. middlewareErrorHandler    MUST be last — catches everything thrown
7. app.listen(config.api.port)     default 7531
8. hourly cleanup jobs       expired invites/sessions, purge 30-day-old tombstones
```
...
```
validate  →  authorise  →  persist  →  sync outward  →  broadcast
(BadRequest) (Forbidden)  (packages/db) (sync engine)  (SSE / notifyCalendarMembers)
```
...
## Realtime (SSE) — current state

The design: `handlers/stream.ts` holds a `Map<userID, Response>` of open [Server-Sent Events](https://developer.mozilla.org/docs/Web/API/Server-sent_events) connections, and `notifyCalendarMembers(userIds, type, payload)` writes `event_created` / `event_updated` / `event_removed` / `calendar_updated` / `calendar_removed` frames to the affected users. Handlers already call it after every mutation. The client listens via `react-native-sse` (`apps/client/hooks/useEventsStream.ts`).

<Aside type="danger">
**Known gap — realtime is currently not wired.** `handlerStream` is imported in `index.ts` but **no route is registered for it**, while the client connects to `/api/stream`. That endpoint 404s today, so live updates don't arrive — the client relies on pull-to-refresh delta sync instead. Wiring it is a good first contribution: register `app.get("/api/stream", requireAuth, handlerStream)` (note: it lives *outside* the `/api/v1` prefix, matching the client URL) and verify the `notifyCalendarMembers` broadcasts land. Confirm the SSE auth story, since `<EventSource>` sends a Bearer token.
</Aside>
...
| Events | `GET/POST/PUT/DELETE /api/v1/events`, `POST /api/v1/events/:id/link`, `/fork` |
...
| Connections | google check/revoke, caldav check/connect/disconnect, `POST /users/connections/disconnect` |
