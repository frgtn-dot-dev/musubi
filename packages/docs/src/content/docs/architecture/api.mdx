...
```mermaid
flowchart LR
    V["validate"] --> A["authorise + preflight"] --> P["commit event + outbox"] --> B["broadcast local commit"] --> S["claim + deliver"]
```
...
- **Identity fields (`creatorID`, `originCalendarID`) are set server-side**, never trusted from the client body. Updates accept the strict content patch schema; they cannot rewrite ownership.
...
`Forbidden` before reading calendar/member data for a non-member. Every new
resource route still needs an explicit authorization gate and a non-member
regression test.
</Aside>
...
`GET /events/:id/attendees` and `PUT /events/:id/attendance` (`{ status: "going" | "maybe" | "declined" | "none" }`, idempotent — the client sends desired state). Both gate on `assertCanViewEvent` (membership in *any* linked calendar), so viewers may answer too. Responses carry `{ id, name, image, status }` only, no emails: an event can span calendars whose members aren't mutuals. Attendance changes broadcast an `attendance_changed` SSE frame (`{ eventID, attendees }`) to members of all linked calendars, so an open detail modal updates live; the client keeps lists in `useAttendeesStore`, fetched fresh on modal open.
...
1. **Write the handler** in the right `handlers/*.ts` file. Follow the mutation boundary — validate (throw `BadRequestError`), authorise (`assertCan*` → `ForbiddenError`), persist (a `@musubi/db` query, throw `NotFoundError` on missing), prepare provider writes before commit, persist their `deliver.outbox` in the local transaction, then claim and deliver with truthful committed receipts, broadcast (`notifyCalendarMembers`), respond (`201` create / `200` update).
