...
<Aside type="note">
Social sign-in here is **id-token based**, not the OAuth authorization-code redirect flow. The native SDK on the device produces a signed token; the client hands it to the server, which verifies it. This is a different thing from *connecting a Google Calendar for sync* (that one **is** a redirect flow — see [below](#calendar-connection-is-not-sign-in)).
</Aside>
...
The sign-in screen awaits `authClient.requestPasswordReset` inside the shared `TextInputModal`; the modal remains open and renders a friendly error when the request fails. Success uses non-enumerating copy ("If an account exists…"). The SMTP email links to **`{BETTER_AUTH_URL}/reset-password`** — a self-contained page the API serves from its own origin (`apps/api/src/handlers/pages.ts`), so self-hosters don't depend on the central website. That page submits the token and new password to Better Auth on the same origin.

Account deletion is **email-confirmed**. From Settings the app requires the exact display name and calls `DELETE /api/v1/users` while the session is live; that only *triggers* Better Auth's `sendDeleteAccountVerification` — no data is removed yet, and the user stays signed in. The email links to **`{BETTER_AUTH_URL}/delete-account`** (also served by the API), whose confirm button POSTs the token to `/api/v1/users/delete/confirm`, which then deletes the user, sessions, and accounts. The token is the credential (no session needed), so the link works in any browser.

The public **`musubi.pro/delete-account`** route on the marketing site is a separate, manual fallback for someone who cannot access the app at all. It submits to the website's `/account/delete-request` endpoint, which emails a request to support through Resend. It **does not delete by unauthenticated email address**; support first verifies ownership, then completes deletion within the published 30-day window. Configure `ACCOUNT_REQUEST_FROM_EMAIL` and `ACCOUNT_REQUEST_TO_EMAIL` in the website environment alongside `RESEND_API_KEY`.
...
The production profile must also receive a direct
`EXPO_PUBLIC_IOS_APP_STORE_URL`. It is used by the minimum-version update
screen, and config evaluation fails when the value is missing or does not end
in a numeric App Store id.
</Aside>
