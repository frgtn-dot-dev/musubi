...
- **Shared screen chrome** lives in `constants/layout.ts`: tab headers use one 56 dp height, while the icon-only tab bar derives its total height from a 44 dp item plus the device bottom safe area. The docked composer and root toast use that same calculation; do not reintroduce a hard-coded tab-bar height in either component.
