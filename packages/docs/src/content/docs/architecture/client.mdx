...
Rules arrive with the refresh (`api.getReminders()` → `storeReminderRules`), are cached in `sync_meta` so a cold offline start can still schedule, and are re-fetched when the SSE frame `reminders_updated` says another device changed them. `deviceTimezone()` (in `lib/timezone.ts`) is reported to the server on every settings refresh — all-day reminders are a wall-clock time, and no server can place "the evening before at 18:00" without knowing whose evening.
