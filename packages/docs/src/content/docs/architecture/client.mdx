...
```
1. GestureHandlerRootView  → ServerProvider → ToastHost   (provider shell)
2. useMigrations(db, …)     run local SQLite migrations
3. load fonts               InterTight, NotoSerif, ShipporiMinchoB1 (kanji)
4. authClient.useSession()  is there a session?
5. version check            GET /api/v1/server, compare semver
6. ready = fonts && !pending && versionChecked && migrated
7. navigate                 session ? /(tabs) : /(auth)/welcome
```
...
`hooks/useEventsStream.ts` opens an `EventSource` (`react-native-sse`) to `/api/stream` and routes frames to the `local*` store methods. See the [known realtime gap](/docs/architecture/api/#realtime-sse--current-state) — the server route isn't wired yet, so today the app leans on pull-to-refresh. There's no auto-reconnect loop; a dropped stream resyncs on the next manual refresh.
