...
- **`services/federation.ts`** holds the federated-server registry: member tokens for calendars shared from *other* Musubi servers, and a `calendarID → origin server` map that `useApi` consults to route writes. It rotates legacy tokens immediately and 90-day credentials during their final 14 days, then persists the successor through the home server. Calendars from federated servers carry `provider: "musubi"` + `serverUrl` and sync in `useRefreshData` after the home pull. Full picture in [Federation](/docs/architecture/federation/).
...
**The server owns the rules; the device owns the ringing.** Which events remind is a three-level chain — the user's global default, then their rule for a calendar, then an override on one event — stored server-side so it roams to every device they sign into (see [Reminders](/reference/reminders/)). `services/notifications.ts` is the executor: it asks `resolveReminders` from `@musubi/calendar` what falls due, hands that to `expo-notifications`, and keeps one SQLite receipt per **occurrence** `{ occurrenceID, eventID, identifier, triggerDate }`.

Receipts are per occurrence, not per event: keying by event meant a daily standup only ever had its next instance scheduled, so an app left unopened for a week reminded once and went quiet. `syncScheduledReminders(events)` diffs the wanted set against the receipts — a full pass also cancels any pending notification no receipt accounts for, which is what clears orphans left by a crash between scheduling and its insert. Pass `{ onlyEventIDs }` when reacting to a single changed event; without it a partial list reads as "everything else is gone".

Rules arrive with the refresh (`api.getReminders()` → `storeReminderRules`), are cached in `sync_meta` so a cold offline start can still schedule, and are re-fetched when the SSE frame `reminders_updated` says another device changed them. `deviceTimezone()` (in `lib/timezone.ts`) resolves all-day reminders here, and is reported to the server on a settings refresh it was making anyway — groundwork for the phase-2 dispatcher, which cannot place "the evening before at 18:00" without knowing whose evening.

**Reminders are exact alarms.** `expo-notifications` only asks for one when the
app is allowed to — otherwise it silently falls back to `setAndAllowWhileIdle`,
which Doze can sit on. `plugins/withExactAlarms.js` declares `USE_EXACT_ALARM`
(Android 13+, granted automatically, and Play review checks the app really is a
calendar) plus `SCHEDULE_EXACT_ALARM` capped at SDK 32 for Android 12. Without
both, "10 minutes before" is a suggestion.

A reminder carries **two buttons**: "Can't make it", which writes the decline
through the ordinary attendance API and therefore also silences the event by the
shared rules, and "Show in calendar". `hooks/useNotificationActions.ts` handles
them and the plain tap; it is mounted at the tabs layout because the app can be
launched cold by a notification, and it reads the events store when the response
arrives rather than closing over what was there when it mounted.

Android reminders go to **one channel per kind** — `musubi-reminders-timed` at
MAX importance and `musubi-reminders-allday` at DEFAULT. A meeting in ten
minutes should interrupt; a birthday tomorrow evening should not have to, and on
Android that distinction lives in the channel, because importance and sound
belong to the user once a channel exists. One channel would have forced one
answer for both, so the only way to quieten all-day reminders would have been to
quieten Musubi. Each channel is created the first time its kind is actually
scheduled — declaring both up front would list a notification the app has never
sent. The old single `musubi-reminders` channel is deleted rather than reused,
since the OS ignores changes to an existing channel's importance.

On iOS a reminder is sent at the **time-sensitive** interruption level, so it
goes through Focus. Reminders only — an entitlement spent on everything stops
meaning anything, and people switch the app off wholesale instead.

Notification permission is **contextual**: the app never prompts at launch. `AddEventModal` calls `requestEventNotificationPermission()` only while saving an event whose reminder toggle is on. The event API write completes before the reminder rule is written, so denial or a native scheduling failure cannot fail event creation/editing; the user gets a toast instead. The composer opens on the rule the event will actually get, inherited or not, and only writes an override where the answer **differs** from what it inherits — always writing one would turn every event into an exception and stop later calendar changes from reaching any of them.

## Help and diagnostics

Settings exposes `Feedback & Roadmap`, `Report a Problem`, Privacy, Terms, and the native version/build. Public ideas open `feedback.musubi.pro`, where users can suggest, vote, and follow planned work. Private problem reports open a user-controlled email draft to `hello@frgtn.dev`, prefilled only with app version/build, platform/OS version, and the selected Musubi server. Calendar names, events, account email, and other user content are never attached automatically. Link-opening failures fall back to a toast with a copyable destination.
