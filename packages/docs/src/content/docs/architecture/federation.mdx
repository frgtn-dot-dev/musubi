...
sequenceDiagram
    ...
    Note over U: stores {server: A, token: M} in SecureStore
...
| Accept handshake | `apps/api/src/handlers/federation.ts` | `POST /api/v1/federation/accept` — public; verifies the invite, find-or-creates the shadow user, adds membership (viewer), returns the raw member token once |
...
| Public preview | `apps/api/src/index.ts` | `GET /api/v1/calendars/tokens/:token` is public — possession of the (unguessable, expiring) invite token is the credential |
...
| Piece | File | What it does |
|---|---|---|
| Registry | `apps/client/services/federation.ts` | federated accounts `{server, token, userID}` in SecureStore; `calendarID → account` origin map; `fedFetch` (authed JSON), `acceptRemoteInvite`, `fetchRemoteCalendarPreview`, `syncFederatedAccounts` |
...
- **The member token is the security boundary.** 32 random bytes, stored only as a SHA-256 hash, returned exactly once. Presenting it authenticates you *as that shadow user* — nothing more.
...
<Aside type="caution">
Known v1 gaps, deliberate and documented: no rate limiting on the accept endpoint; no server-to-server identity proof (the invite token is the proof); member tokens don't expire (revoked implicitly by kick, `revokeMemberTokens` exists for cleanup); a shadow user with no remaining memberships lingers until cleaned up; and **member tokens live per device** (SecureStore) — accepting on your phone doesn't federate your tablet; each device accepts the invite itself. Token roaming is one of the main reasons routing moves server-side in v2 (the home server holds the relationship, all your devices inherit it — at the cost of the home server being able to act on your behalf). All are contained by the authorization-is-membership rule.
</Aside>

## What v1 is — and isn't

**v1 (this)**: the remote calendar lives *only* at its origin. The invitee's app reads and writes it there directly; a full pull per refresh keeps a device-local cache (SQLite) like any other calendar. If the origin is unreachable, the app shows the last synced copy and edits fail loudly.
