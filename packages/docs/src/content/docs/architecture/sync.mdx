...
```
                         ┌──────────────── engine.ts ────────────────┐
   external providers     │  registry: { google, caldav, … }          │   local DB
   ┌──────────┐  pull     │                                           │  ┌──────────────┐
   │  Google  │ ───────►  │  syncProvider(adapter, user, account):    │  │ external_*    │
   │  CalDAV  │           │    1. listCalendars → reconcile mirrors   │─►│ events        │
   │  (…)     │ ◄───────  │    2. fetchChanges(cursor) → upsert       │  │ calendar_*    │
   └──────────┘  push     │    3. push local edits via pushCreate/... │  └──────────────┘
                         └────────────────────────────────────────────┘
```
