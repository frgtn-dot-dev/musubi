...
K07 preserves pending/attempting intent across process exit, but does not yet
retry it. Unresolved predecessors block later attempts. K08 owns restart recovery,
provider-specific idempotence/reconciliation, pull coordination and disconnect
cleanup. This is not a distributed transaction or a guarantee of eventual delivery.
...
| Scheduled pull | Provider fetch, then local reconciliation | Account/provider failures are isolated and retried next cycle; pending-write coordination remains K08 |
