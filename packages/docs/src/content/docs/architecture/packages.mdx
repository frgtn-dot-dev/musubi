...
```
packages/config ◄── everything (env)
packages/types  ◄── apps/client, apps/api, packages/db   (the contract)
packages/db     ◄── apps/api, packages/auth              (server-side only)
packages/calendar ◄── apps/client, apps/api              (recurrence)
packages/auth   ◄── apps/api
```
