...
| `eventShares` | Token-gated public event page | publication mode, attendee visibility, closed `theme` JSONB, and validated `content` JSONB for cover placement, display tags, and agenda |
...
`S3_BUCKET` is set, or to the persistent local media volume otherwise. Public
avatar endpoints and client URLs stay unchanged. Uploaded event covers use the
same storage abstraction under an event-scoped key; `eventShares.content` stores
only whether that cover is active and its focal point, never image bytes or an
S3-specific URL.
