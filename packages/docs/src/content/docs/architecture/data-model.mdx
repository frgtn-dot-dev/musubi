...
| Table | Purpose | Notes |
|---|---|---|
| `memberTokens` | Bearer tokens for federated shadow members | `tokenHash` unique (SHA-256 — the raw token is never stored); cascades from `user`. Authentication only — authorization stays with `calendar_members`, so a kick cuts access. See [Federation](/docs/architecture/federation/). |
