...
The root manifest pins `pnpm@11.8.0` and is the only toolchain source:
...
Do not install a separate global pnpm or add package-local `packageManager`
fields.
...
**Symptom:** config evaluation reports that
...
The numeric id must be the real App Store Connect app id. Development and
preview builds can omit the variable and use the public download page fallback;
production intentionally cannot.
...
server advertises `email: false` when the configured SMTP connection or
authentication handshake fails, and the client hides or adapts the UI. The
handshake runs once before the API listens; restart it after changing SMTP.
