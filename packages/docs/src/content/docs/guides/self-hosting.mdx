...
<Aside type="tip">
Password reset and account-deletion confirmation are fully self-contained: your API serves those pages from its own origin (`{BETTER_AUTH_URL}/reset-password` and `/delete-account`), and the emails link there. Nothing depends on musubi.pro — you only need SMTP configured so the emails can go out.
</Aside>
