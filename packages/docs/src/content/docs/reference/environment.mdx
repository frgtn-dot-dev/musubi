...
| `FEDERATION_ALLOW_PRIVATE_HOSTS` | Only to federate with a Musubi server on a private address | `true` to allow; anything else refuses. Defaults to allowed outside `ENVIRONMENT=prod` |
