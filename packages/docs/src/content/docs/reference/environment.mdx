...
| `PUBLIC_HOST` | Full stack only | Hostname the gateway serves — a domain gets automatic HTTPS, `:80` serves plain HTTP behind another proxy. Defaults to `:80` |
...
`dokploy-network`. The full stack (`docker-compose.full.yml`) publishes only the
gateway: the web and API containers stay on the Compose network, so there is no
second origin that could bypass it.
