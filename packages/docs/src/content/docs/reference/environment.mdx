...
| `PUBLIC_HOST` | Full stack | Hostname the stack serves — Caddy takes a domain and gets HTTPS automatically, `:80` serves plain HTTP behind another proxy (defaults to `:80`). On Dokploy it is the hostname the Traefik routers match and is required |
...
| `TRAEFIK_CERT_RESOLVER` | No | Dokploy only — name of the Traefik certificate resolver. Defaults to `letsencrypt`, which is what a stock Dokploy install uses |
...
proxy to sit in front of. The Dokploy stack publishes nothing — Traefik reaches
both services over `dokploy-network`, where the API also keeps the stable
`musubi-api` alias for scraping.
