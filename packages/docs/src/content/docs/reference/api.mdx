...
| `GET /api/v1/public/events/:token/rsvp` | Read the authenticated guest's answer | 60 / 15 min / IP |
| `PUT /api/v1/public/events/:token/rsvp` | Write the authenticated guest's answer | 30 / 15 min / IP |
...
| `GET /api/v1/events/:eventId/rsvps` | — | Organizer view of answer counts and names |

Published event reads are open to possession of the token. RSVP reads and writes
require a session; the public page authenticates guests with an emailed code
