...
| `GET /api/v1/public/events/:token/rsvp` | Read public guest counts/names allowed by the organizer, plus the caller's answer when authenticated | 60 / 15 min / IP |
| `PUT /api/v1/public/events/:token/rsvp` | Write the authenticated, verified-email guest's answer | 30 / 15 min / IP |
| `GET /api/v1/public/events/:token/cover` | Read the active uploaded event cover; token is the capability | 120 / 15 min / IP |
...
| `reminders_updated` | Refetch reminder rules and reschedule; carries no payload |
...
Page, settings, and reminder events fan out only to the acting user's own sessions,
...
| `PUT /api/v1/events/:eventId/share` | `{ attendeeVisibility, content, theme }` | Create or update the published page, tags, agenda, cover focal point, and token URL |
| `PUT /api/v1/events/:eventId/share/cover` | JPEG, PNG, or WebP bytes (max 5 MB) | Store or replace the uploaded cover in configured media storage |
| `DELETE /api/v1/events/:eventId/share` | — | Revoke the public token |

Published event reads and organizer-approved attendee counts/names are open to
possession of the token. Reading one's own RSVP and writing any answer require a
verified-email session; the public page authenticates guests with an emailed code
...
| `POST /api/v1/calendars/invites/:inviteId/send` | `{ email }`; mails the EXISTING link, so revoking still kills every copy. Needs `invite` on that calendar and is capped at 20/hour per **account** — an IP cap would punish an office and miss a roaming phone |
...
real — see [Reminders](/reference/reminders/).

## Reminders

| Method and path | Request / response |
| --- | --- |
| `GET /api/v1/reminders` | `{ default, calendars: {id: rule}, events: {id: rule} }` — only explicit choices |
| `PUT /api/v1/reminders/calendars/:calendarId` | `{ rule }`; `null` inherits the global default. `403` for a non-member |
| `PUT /api/v1/reminders/events/:eventId` | `{ rule }`; `null` clears the override. Requires only that the caller can *see* the event |
| `DELETE /api/v1/reminders/events/:eventId` | Same as `PUT` with `rule: null` |
...
Sent because a **person** did something, as opposed to a reminder, which is a
promise you made to yourself. Two switches in `settings.notificationEmails`
(`eventChanged`, `pollDecided`), both on by default.

| What | When | Batched |
| --- | --- | --- |
| An event moved or was cancelled | Only the start/end changing or `isCanceled` going true. Guests who have not declined, minus whoever made the change | Yes — `pending_notifications`, one email per person per batch |
| A poll has a time | `POST /api/v1/scheduling/polls/:pollId/decide` | No — a poll is decided once, and half the participants may have no account |
