...
| `PUT /api/v1/public/polls/:token/votes` | Write a verified participant's complete answer set | 60 / 15 min / IP |
...
| `POST /api/v1/scheduling/polls` | Required | Create a poll from `title`, day `slots`, and optional `description`, `deadline`, `approximateStartTime` |
| `GET /api/v1/public/polls/:token` | Optional | Read the public poll projection and the caller's own answer when identifiable |
| `PUT /api/v1/public/polls/:token/votes` | Required | Replace one participant's complete `yes` / `if-needed` / `no` answer set |
...
Creation and answers require a verified email OTP. An existing account's saved
name is reused; a new account chooses one after verification. The public
projection contains participant names and answers, never email addresses or
