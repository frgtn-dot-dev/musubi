...
| Method and path | Request / response |
|---|---|
| `GET /api/v1/pages` | Active Pages ordered by position; lazily backfills the default Page on first read |
...
| `GET /api/v1/users/connections/musubi` | Session | Decrypted remote connections for the owner — **deprecated**, see the gateway below |
| `POST /api/v1/users/connections/musubi` | Session | `{ server, userID, token }`; the origin is SSRF-checked before it is stored |
...
limits.
Returning the decrypted token from `GET …/connections/musubi` stays only until
the mobile client has migrated onto the gateway.
