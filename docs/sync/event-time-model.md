...
### Internal local time CAS checkpoint

`replaceLocalEventTimeAtRevision` resolves an explicit time intent under the authoritative event row lock after checking the expected revision. It acquires calendar lifecycle fences first and revalidates the current origin/membership before proceeding. It commits start/end/isAllDay/timeModel and one revision increment together, preserving all unrelated fields. Canonically identical time values are no-ops; stale input conflicts before time parsing. The existing recurrence is validated through known-model expansion even for cancelled definitions.

This first writer is strictly local: a detached identity, any child (including a tombstone), external calendar association, external mapping or retained outbox history requires the later occurrence/provider path and is rejected before mutation. No external enqueue, network delivery, public route or DTO admission is enabled. The future handler must authorize the event and publish the committed snapshot; this DB helper does not grant access. PostgreSQL tests cover durable coherent values, no-op, stale/invalid rollback, two competing CAS edits, cancellation validation, all-day conversion and unsupported-family/provider guards.


### Occurrence detail temporal tuple checkpoint

Native presentation and live content refresh preserve the selected occurrence's start, end, isAllDay and timeModel together. Copying only its instants onto the master's civil anchors made the subsequent editor show the series anchor date. The composer continues to freeze separate master and occurrence snapshots. A content-only whole-series scope update restores the master's timeModel along with its instants in the optimistic snapshot; the generic wire patch still contains only the actual changed content and the frozen revision.

Integration coverage passes real shared zoned/floating/all-day expansion through detail, live rename, composer, All events save, pending optimistic store and authoritative response. This is a prerequisite correction, not activation of temporal scope writes, occurrence identity writes or provider delivery.


### Civil whole-series shift checkpoint

For a known local plain-RRULE series with unchanged kind and zone, All events applies the edited occurrence's civil displacement separately to the stored master start and end anchors. The resulting anchors are resolved together by the existing explicit time resolver, retaining its DST policy and validating actual order. Because both civil endpoints move, a shift across an offset transition can change elapsed duration; recurrence expansion continues to use the resulting master's elapsed duration for zoned events. Hidden precision is retained by the draft unless the minute field was changed.
