...
| K14 Google intervalová dostupnost | Default-off freeBusy.query API, privátní registry zdrojů a explicitní výběr do 20 zdrojů v Connections; UTC intervalový dialog bez Event identit či edit akcí | Scope/owner/account/generation kontroly, reconnect-required versus unavailable versus potvrzené prázdno, bez offline cache. Grid a nativní zobrazení intervalů zůstávají implementační práce, nikoli automaticky blokované lidským schválením. Živý re-consent a aktivace zbývají. [Kontrakt](../sync/google-availability.md) |
