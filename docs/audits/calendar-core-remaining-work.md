...
| K12 CalDAV | Osobní whole-resource series content/time/RRULE změna, occurrence content/time/cancel/revival, series/following delete a dvoukrokový following update | Známý zoned/floating/all-day model a úplná resource evidence; resource write, collection bind/unbind dle operace. Explicitní potvrzení master/active-child content a uloženého series/active-child time či series RRULE záměru je podporované při shodné původní nativní struktuře. Generated/cancel/revival potvrzení zachovává původní definice a intent; Partial following-delete konflikt má přesné potvrzení bodu řezu, kontrolu tombstones a ochranu proti opožděnému starému syncu. Whole-resource delete konflikt má samostatné potvrzení celé série a collection-unbind proof. Source konflikt splitu před prvním ACK má potvrzení obou operací se stejnou budoucí identitou; konflikty po source ACK a širší převody zbývají. [Kontrakt a Radicale důkazy](../sync/caldav-series-writes.md) |
| K12 Graph | Provider-expanded import s původní identitou; přesné forward/reverse kandidáty šesti běžných recurrence patternů a striktní master-time kandidáty s kontrolou zóny, DST a přesnosti | Recurring create, master echo dedup a nativní scope delivery nejsou zapojené. Samotný converter není write capability. Read-only create-recovery kandidát ověřuje úplnou scoped transaction listing a fresh master bez výjimek/cancellations; soukromý transport ověřuje jediný POST a obnovu po ztracené odpovědi, zatím bez zapojení fronty a importu. Soukromý create vyžaduje úplnou konečnou COUNT sérii do 366 výskytů / 730 dnů s ověřenými původními časy. Read-only family kandidát načítá celý konečný rozsah, přesunuté výjimky a explicitní cancellation evidence; čas výjimky bez důkazu aktuální zóny zachovává jako unknown. Sdílená expanze už čte přesné UTC výjimky s explicitně unknown zónou bez duplikace původního slotu; privátní atomický DB import už zachovává identity/cancellations a kontroluje souběhy. Sync už načítá existující známé rodiny odděleně od calendarView a zachová je při resetu i vypnutém editačním flagem. Ověřená absence masteru atomicky odstraní místní family; úplná obnova zachová původní UUID i cancellations. Běžný import také kontroluje nově přijatý master pod lifecycle lockem i pro dosud nenamapované výskyty a vypnutý flag. Privátní default-off create journal atomicky ukládá záměr, drží stabilní transaction UUID a blokuje předčasný import včetně souběhu během fetch. Privátní full-family ACK už atomicky ukládá master/výskyty a uzavírá konkrétní lease, včetně rollbacku při kolizi nebo expiraci. Default-off worker už propojuje jediný POST, obnovu transakce, úplné čtení a ACK; ověřeno přes actual adapter/fake HTTP/DB včetně timeoutů a souběhů. Veřejný create a živá acceptance ještě nejsou zapojené. [Příprava create](../sync/graph-recurring-create.md). [Import](../sync/provider-time-import.md#graph-occurrence-mapping-slice), [kandidáty](../sync/event-scope-operations.md#graph-recurrence-candidates) |
