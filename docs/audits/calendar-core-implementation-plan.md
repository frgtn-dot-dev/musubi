...
- K12 Graph tracked-master removal/revival: two complete exact-master 404 responses around a successful exact-calendar read permit context-checked atomic local family tombstones. Failed/partial/access-denied reads preserve data and cursor; retained mappings support full-proof revival under original UUIDs, preserving native cancellations. HTTP/PostgreSQL regressions cover stale deltas/reset, repeated no-op and local races. This is read reconciliation, not native DELETE or permission to repeat POST; pending/new-create coordination and durable ACK remain open.

- K12 Graph late-family import fence: native parent addresses survive ordinary flag-off reads as private admission hints. Under the calendar lifecycle lock, an unmapped component is skipped if its complete canonical family was accepted after fetch exclusions were captured. Adapter/PostgreSQL regression creates this interleaving and confirms no independent echoes before the next complete read. The hint does not synthesize canonical identity; pending create without a master address and durable ACK remain open.

- K12 private Graph create journal: default-off atomic personal root + frozen native projection, exact mutation replay and durable existing lease uncertainty. Current permission/source/lease checks refuse changed state; ordinary import and family enumeration wait for pending creation, including admission during fetch. Generic master-only ACK is refused. PostgreSQL regressions cover replay, rollback, cursor safety and local/source/lease fences. Native delivery, full-family ACK and public create remain pending.

- K12 private Graph full-family ACK: exact live lease and current source checks now guard one atomic master/children/mappings/journal transaction. Complete unchanged personal footprint is required; partial/changed/duplicate/tombstoned/colliding observations and mid-transaction lease expiry roll back everything. Timed DST and all-day year-boundary PostgreSQL regressions include concurrent one-winner ACK and subsequent family sync no-op. Native delivery/recovery and public create composition remain open; production flags are unchanged.

- K12 default-off Graph create worker: frozen journal, current local/native permission, one POST, stable transaction recovery and strict full-family ACK are now composed in the specialized path. Actual adapter/fake HTTP/PostgreSQL regressions cover concurrent claims, timed/all-day creation, lost/invisible responses, partial reads, timeout/lease recovery, denial/races and flag-off echo no-op. Uncertain absence never reposts; public admission and live acceptance remain open.

- K12 default-off Graph public create: strict known-time personal finite series admission now queues durable intent and returns HTTP 202 without provider I/O. Stable explicit/default UUID retries preserve one operation, return current readable state and never resurrect deleted events. Real HTTP/PostgreSQL coverage includes post-commit failure and permission-loss receipts. Client/browser and live acceptance, wider recurrence and native scope writes remain open.

- K12 creation retry identity: web drafts retain their UUID through submission failure, quick/full handoff and full-editor reload; native composers retain it through failed/edited retries and reset for a new draft. Mock browser 202/lost-response acceptance covers finite zoned/all-day creation, with native callback regressions. Real Outlook and physical native acceptance remain separate; production flags are unchanged.

- Creation retry follow-up: ordinary legacy/local-known endpoints now reconcile an existing own readable UUID before fresh admission. Current matching content returns 202 without another journal/write; changed/deleted/unavailable content returns committed 409. Concurrent first requests converge after insertion. This is current-state reconciliation while canonical state is retained, not an immutable historical request ledger. HTTP/PostgreSQL regressions cover all three time models and uncertain native history preservation.

- K14 Google existing-instance reminder native candidate: default-off own-primary transport validates native parent/original identity and known current time, freezes a reminders-only intent and sends at most one conditional PATCH. Complete GET confirmation/recovery preserves unrelated fields; fake HTTP covers lost responses, malformed confirmation, native/local denial and races. Durable binding/queue/ACK, conflict and client composition remain open; no production flag or live account is changed.

- K14 private Google instance reminder journal: prepare/commit freezes full native evidence with accepted parent/mapping/original-slot context, revalidates local authority after external preflight and records exactly one personal intent without canonical changes. PostgreSQL regressions cover concurrent replay, parent/child/mapping races, grant and lease loss, and refusal by generic worker/ACK/resolution. Specialized delivery, public admission, explicit conflicts and client acceptance remain open.

- K14 Google instance reminder delivery: specialized default-off worker composes exact journal/native verification, live source checks before I/O/PATCH, complete read recovery, and atomic parent/mapping/permission/lease ACK. Actual fake HTTP/PostgreSQL regressions include lost responses, ACK storage failure, concurrent workers, native/local races and pull preservation; canonical content/revision stays unchanged. Public admission, explicit conflicts and client acceptance remain open.

- K14 Google instance reminder conflicts: full native refresh and opaque baseline/parent hash now support explicit reminders-only replacement or read recovery. Commit revalidates exact source, mapping, original slot and parent context, preserving unrelated current native state. Fake HTTP/PostgreSQL regressions cover stale parent/private fields, reparenting, permission loss, concurrent confirmation and one-off-proof refusal. Public admission and client acceptance remain open.

- K14 public Google instance reminders: strict authenticated admission performs native preflight outside DB locks, returns private durable 202 and replays without another native read. Web/native editors refresh capability, label the occurrence and preserve failed requests; web occurrence/series delivery histories target distinct identities. Actual HTTP/PostgreSQL, client component and six browser/mock scenarios cover zoned/all-day, light/dark, narrow/wide, focus/accessibility and retry. Live provider/physical OS acceptance remains open; production flags stay off.

- Current grant for explicit delivery actions: retry, conflict preview and fresh confirmation now require the connected owner's current editor/owner membership. Mutating admission holds the membership/source rows against a concurrent downgrade. HTTP/PostgreSQL regressions reproduce the former viewer retry, deny stale confirmation without native reads or journal changes, and preserve read-only delivery receipts and locally authorized native read recovery. This does not close general privacy downgrade/regain or automatic worker revocation.

- K14 Google access-generation fence: complete discovery now atomically versions local access transitions and invalidates the cursor. Actual HTTP/PostgreSQL interleavings reject old-generation upsert, delete, sweep and cursor commits, including ABA, while fresh full reads preserve retained UUIDs and reconcile omissions with time editing disabled. This is the ordering prerequisite only: immediate privacy redaction after failed reads and public/client/notification invalidation remain open. See `docs/sync/google-calendar-access.md`.

- K14 Google mirror redaction: limited-access discovery atomically retires readable mapped-origin details/provider state, preserving temporal identities, links and saved local intents. Failed-fetch and fresh-only-regain DB regressions include linked readers and immediate SSE; revision-keyed web/native details plus four browser/mock scenarios retire open private observations and return focus. Historical receipt titles and concurrent notification payloads remain a separate privacy follow-up. Production flags and minima remain unchanged.

- Privacy review follow-up: private mapping and personal-operation recovery markers preserve fresh-only read restoration without losing pending RSVP/reminder intent. Explicit confirmation alone creates a replacement; native-time/source/mapping/intent/lineage fences remain. One-off reminders recheck the live source and lease after their final native preflight. Linked readers receive committed restoration, and privacy read restoration creates no linked provider fanout. HTTP/DB regressions cover all four personal paths in DST/all-day and stale/multiple-pending refusal.

- K14 privacy batch: authoritative Google source absence/freeBusyReader redacts shared tombstones atomically with normal removal and wakes previous readers. Historical inbox titles require current exact full-grant source and fresh cursor; queued event-change emails revalidate live visibility/access and refresh each batch before stub-tested transport. Private intents remain unchanged. The limited-read restoration marker no longer suppresses later genuine inbound fanout. Full-editor/generated caches, physical native/OS behavior and wider free/busy remain open.
