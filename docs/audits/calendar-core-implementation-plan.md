...
**K10 jednotná reminder projekce — rozpracovaný balíček:** tři dosavadní explicitní projekce používají společný `toReminderEvent`; web tak zachová stejná metadata jako mobil a server. Sdílené regrese nyní zahrnují tento převod před expanzí a původní nativní SQLite/serverová PostgreSQL regrese dále ověřují skutečné hranice konzumentů. Root check, cílená DB regrese a čisté nezávislé review prošly. Obecné DTO, hlášení chyb konzumentů a writable aktivace zůstávají pending.
