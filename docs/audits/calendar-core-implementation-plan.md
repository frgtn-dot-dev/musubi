...
**Stav: K06 ROZPRACOVÁN — NEPŘEVZATO, NEDOKONČENO, NENASAZOVAT.** DB CAS základ není zapojen do API/klientských save cest; stale-draft ochrana ani úplné conditional delivery zatím nejsou dodány. Rodič převzal pouze tři ověřené dílčí řezy po nezávislé revizi a obnově finalizace. Toto převzetí neznamená dokončení zbývajícího K06. Celkové K06 acceptance zůstává nesplněné; samostatné pokračování potřebuje zbytek wire/API/client/provider integrace a regresní důkazy.
...
K05 je převzat. **K06 má převzat pouze dílčí checkpoint; K07–K15 zůstávají pending.** Další práce je skutečný revision/PATCH wire kontrakt a CAS všech handlerů, zmrazené revize webových/nativních draftů včetně scope handoffů, Google/CalDAV conditional diff delivery a pravdivé postcommit konflikty. Kompatibilita je rozhodnuta: produkt, minimum klienta i peeru 0.1.8; Outlook update/delete dočasně blokovat podle schválené hranice. K06 dokončit a nezávisle ověřit před jakýmkoli nasazením.
