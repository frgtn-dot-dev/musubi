...
**K10 ochrana legacy časových zápisů — rozpracovaný balíček:** skutečný diff start/end/isAllDay/recurrence se kontroluje pod event lockem po revision CAS. Známý model nebo detached identita vyžadují budoucí explicitní časový/scope kontrakt; starý PATCH ani provider pull nesmí změnit čas a ponechat stale metadata. Odmítnutí předchází obsahu, vazbám a přijetí nového ETag. Title-only a časový no-op metadata zachovají; unresolved legacy chování zůstává. Guard sám neaktivuje DTO, provider serializéry, fork ani scope/cascade operace.
