...
import { db, markProviderOrganizer, type EventOutboxRow } from "@musubi/db";
...
export async function verifyOrganizerDispatchLocks(
  row: EventOutboxRow,
  mutate: (tx: Transaction) => PromiseLike<unknown>,
) {
  const name = `google_organizer_lock_${randomUUID().replace(/-/g, "")}`;
  ...
  await db.execute(
    sql.raw(
      `CREATE FUNCTION ${name}() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.id = '${row.id}' AND NEW.payload->'organizer'->'dispatch' IS NOT NULL AND OLD.payload->'organizer'->'dispatch' IS NULL THEN PERFORM pg_advisory_xact_lock(${key}::bigint); END IF; RETURN NEW; END $$`,
    ),
  );
  await db.execute(
    sql.raw(
      `CREATE TRIGGER ${name} BEFORE UPDATE ON event_outbox FOR EACH ROW EXECUTE FUNCTION ${name}()`,
    ),
  );
  ...
  let marking: Promise<boolean> | undefined,
  ...
  try {
    ...
    marking = markProviderOrganizer(row)
      .then(() => true)
      .then((result) => {
        markerFinished = result;
        return result;
      });
    await waitFor(async () => {
      if (markerFinished !== undefined)
        assert.fail(`Marker completed before gate: ${markerFinished}`);
    ...
    await waitFor(async () => {
      if (committed)
        assert.fail(
          "Source mutation slipped between validation and dispatch marker",
        );
      if (!pid) return false;
      const rows = await db.execute(
