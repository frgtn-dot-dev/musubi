...
async function main() {
  ...
  try {
    ...
    const splitRemote = await davFetch(journalURL, { headers: { authorization: basicAuth } });
    const splitBody = await splitRemote.text();
    assert.ok((await davFetch(journalURL, { method: "PUT", headers: { authorization: basicAuth, "content-type": "text/calendar", "if-match": splitRemote.headers.get("etag")! }, body: splitBody.replace("END:VCALENDAR", "X-MUSUBI-SPLIT-CONFLICT:1\r\nEND:VCALENDAR") })).ok);
    assert.equal((await deliverEventOutbox(journalOperations[0].id, () => caldavAdapter))?.status, "conflict");
    const splitComparison = await prepareEventDeliveryResolution(userID, journalRoot.id, journalOperations[0].id, () => caldavAdapter);
    assert.deepEqual(splitComparison.preview.scopeResolution, { kind: "following-update", originalStart: journalCut.originalStart, newSeriesId: journalSplit.creation.master.id });
    assert.equal(splitComparison.preview.splitFuture?.title, "Durable future family");
    const splitReplacement = await commitEventDeliveryResolution(userID, splitComparison.proof, { mutationId: randomUUID(), expectedLocalRevision: splitComparison.preview.localRevision, expectedLatestOperationId: splitComparison.preview.latestOperationId, expectedRemoteExists: true, expectedRemoteEtag: splitComparison.preview.remoteEtag, expectedScopeResolution: splitComparison.preview.scopeResolution });
