...
export const caldavRsvpFixtureData = [
  "BEGIN:VCALENDAR",
  "VERSION:2.0",
  "PRODID:-//Musubi//RSVP Fixture//EN",
  "BEGIN:VEVENT",
  "UID:rsvp-fixture",
  ...
  "ATTENDEE;CN=Self;PARTSTAT=NEEDS-ACTION;ROLE=REQ-PARTICIPANT:mailto:self@example.test",
  "ATTENDEE;CN=Other;PARTSTAT=ACCEPTED:mailto:other@example.test",
  "X-PRIVATE;X-KEEP=yes:Folded",
  " extension",
  "BEGIN:VALARM",
  "ACTION:DISPLAY",
  "TRIGGER:-PT15M",
...
export const caldavRsvpDstDurationData = caldavRsvpFixtureData
  ...
  .replace(
    "DTSTART:20260328T090000Z",
    "DTSTART;TZID=Europe/Prague:20260328T090000",
  )
  .replace("DTEND:20260328T100000Z", "DURATION:PT24H");
export async function createCaldavOrganizerFixture() {
  const state = {
    data: caldavRsvpFixtureData
      .replace("mailto:self@example.test\r\n", "mailto:guest@example.test\r\n")
      .replace("mailto:organizer@example.test", "mailto:self@example.test") as
      string | null,
    etag: '"before"',
    ...
    onPut: undefined as (() => Promise<void>) | undefined,
  };
  const server = createServer(async (req, res) => {
    state.requests.push(`${req.method} ${req.url}`);
    assert.ok(req.headers.authorization?.startsWith("Basic "));
    if (state.mode === "redirect") {
      res.writeHead(302, { location: "http://foreign.invalid/leak" });
      return res.end();
    }
    const transient = /^(OPTIONS|PROPFIND)-(408|429|503)$/.exec(state.mode);
    if (transient && req.method === transient[1]) {
      res.writeHead(Number(transient[2]), { "retry-after": "17" });
      return res.end();
    }
    ...
    if (req.method === "PROPFIND") {
      ...
      if (req.url === "/collection/")
        ...
        assert.ok(
          req.url?.startsWith("/collection/") && req.url.endsWith(".ics"),
        );
        props = `<d:current-user-privilege-set><d:privilege><d:${state.mode === "no-write" ? "read" : "write-content"}/></d:privilege></d:current-user-privilege-set>`;
      }
      const response = `<d:response><d:href>${state.mode === "wrong-href" ? "/elsewhere" : req.url}</d:href><d:propstat><d:prop>${props}</d:prop><d:status>HTTP/1.1 ${state.mode === "failed-propstat" ? "403 Forbidden" : "200 OK"}</d:status></d:propstat></d:response>`;
      res.writeHead(207, { "content-type": "application/xml" });
      return res.end(
        `<d:multistatus xmlns:d="DAV:" xmlns:c="${state.mode === "wrong-namespace" ? "urn:evil" : "urn:ietf:params:xml:ns:caldav"}">${response}${state.mode === "duplicate-response" ? response : ""}</d:multistatus>`,
      );
    }
    assert.ok(req.url?.startsWith("/collection/") && req.url.endsWith(".ics"));
    if (req.method === "GET") {
      state.reads++;
      await state.onRead?.();
      if (state.data === null) {
      ...
      return res.end(state.data);
    }
    assert.ok(["PUT", "DELETE"].includes(req.method!));
    assert.equal(req.headers["if-schedule-tag-match"], undefined);
    assert.equal(req.headers["schedule-reply"], undefined);
    if (req.method === "PUT") state.puts++;
    else state.deletes++;
    if (
      (state.data === null
        ? req.headers["if-none-match"] !== "*"
        : req.headers["if-match"] !== state.etag) ||
      state.mode === "race"
    ) {
      res.writeHead(412);
      return res.end();
    ...
    state.data = req.method === "DELETE" ? null : data;
    state.etag = '"after"';
    state.scheduleTag = '"schedule-after"';
    if (state.mode === "metadata" && state.data)
      state.data = state.data
        .replace("DTSTAMP:20260301T090000Z", "DTSTAMP:20260302T090000Z")
        .replace(
          "ATTENDEE;CN=Other;",
          'ATTENDEE;SCHEDULE-STATUS="2.0";CN=Other;',
        );
    if (state.mode === "changed-after" && state.data)
      state.data = state.data.replace(
        "Private notes",
        "Concurrent private notes",
      );
    await state.onPut?.();
    if (state.mode === "lost") {
      req.socket.destroy();
  ...
  return {
    ...
    resource: origin + "/collection/invite.ics",
    close: async () => {
