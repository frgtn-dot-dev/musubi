...
async function main() {
  ...
  const owner = `alarm-native-${randomUUID()}`, collection = new URL(`${username}/musubi-alarms-${randomUUID()}/`, process.env.RADICALE_URL).href;
  ...
  try {
    ...
    const request = { operationID: randomUUID(), expectedRevision: event.revision, expectedStateVersion: caldavAlarmVersion(context, observed.data), provider: "caldav", alarms: { minutesBeforeStart: 30 } };
    ...
    console.log("Radicale alarms: native positive privilege, full-resource CAS, preserved bytes, conflict and explicit alarm removal: OK");
  } finally {
    config.api.caldavAlarmEditsEnabled = flag;
    if (created) await fetch(collection, { method: "DELETE", headers: { authorization } });
    await db.delete(user).where(eq(user.id, owner)); await db.$client.end();
  }
}
