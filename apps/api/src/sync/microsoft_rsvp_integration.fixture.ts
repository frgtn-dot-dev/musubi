...
export async function runMicrosoftRsvpIntegration(recurring = false) {
  ...
  try {
    for (const scenario of ["live-tentative", "live-accept-pull", "live-accept-snapshot-mismatch", "live-accept", "live-accept-foreign", "live-accept-content", "mark-lock-role", "mark-lock-revision", "mark-lock-source", "mark-lock-account", "whitespace", "uid-backfill", "decline-pull-before-ack", "all-day", "echo-dst", "public", "accepted", "tentative", "declined", "no-op", "lost", "not-observed", "decline-absent", "changed", "before-network-restart", "echo-before-ack", "flag", "worker-flag", "account-after-read", "role-after-read", "revision-after-read", "lease-after-read", ...(recurring ? ["mark-lock-slot", "slot-after-read", "parent-after-read", "slot-before-ack", "admission-binding"] as const : [])] as const) {
      ...
      try {
        ...
        if (scenario === "uid-backfill" && !recurring) {
        ...
        const request = { operationID: randomUUID(), provider: "microsoft", expectedRevision: original.revision, expectedStateVersion: observation.version!, response, notificationPolicy: "send-response", ...(recurring ? { scope: "occurrence" } : {}) };
        if (scenario === "admission-binding") {
          for (const corrupt of [
            { externalSeriesID: null, originalStart: null },
            { originalStart: null },
            { externalSeriesID: "meeting" },
            { originalStart: { kind: "instant" as const, value: "2026-03-29T08:00:00.000Z" } },
          ]) {
            await db.update(externalEvents).set(corrupt).where(eq(externalEvents.id, mapping.id));
            const current = await getOwnProviderEventObservation(actor, original.id);
            await assert.rejects(() => queueProviderRsvp(actor, original.id, { ...request, expectedStateVersion: current.version! }));
            await db.update(externalEvents).set({ externalSeriesID: mapping.externalSeriesID, originalStart: mapping.originalStart }).where(eq(externalEvents.id, mapping.id));
          }
          await assert.rejects(() => queueProviderRsvp(actor, original.id, { ...request, scope: undefined }));
          assert.equal(fixture.state.posts, 0); assert.equal((await db.select().from(eventOutbox).where(eq(eventOutbox.eventID, original.id))).length, 0);
          continue;
        }
        ...
        if (scenario === "public") {
          ...
          if (recurring) {
            const legacy = await fixture.originalFetch(`${endpoint}/provider-state`, { headers });
            assert.equal((await legacy.json()).rsvpEdit, undefined, "Old clients must not receive a new occurrence capability");
          }
          const publicState = await fixture.originalFetch(`${endpoint}/provider-state${recurring ? "?outlookRsvp=1" : ""}`, { headers }); assert.equal(publicState.status, 200);
          const publicBody = await publicState.json();
          assert.deepEqual(ProviderEventStateResponseSchema.parse(publicBody).rsvpEdit, { provider: "microsoft", expectedRevision: original.revision, ...(recurring ? { scope: "occurrence" } : {}) });
          for (const secret of ["graphIdentity", "externalSeriesID", "originalStart", "graphOccurrence", "master", "baseline"]) assert.ok(!JSON.stringify(publicBody).includes(`"${secret}"`), `Private ${secret} must not leak`);
        }
        ...
        if (scenario.endsWith("after-read")) fixture.state.hook = async () => {
          ...
          if (scenario === "slot-after-read") await db.update(externalEvents).set({ originalStart: { kind: "instant", value: "2026-03-29T08:00:00.000Z" } }).where(eq(externalEvents.id, mapping.id));
          if (scenario === "parent-after-read") await db.update(externalEvents).set({ externalSeriesID: "another-series" }).where(eq(externalEvents.id, mapping.id));
        ...
        if (scenario === "slot-before-ack") fixture.state.hook = async () => {
          if (fixture.state.posts) { fixture.state.hook = undefined; await db.update(externalEvents).set({ originalStart: { kind: "instant", value: "2026-03-29T08:00:00.000Z" } }).where(eq(externalEvents.id, mapping.id)); }
        };
        ...
        if (scenario === "slot-before-ack") { assert.equal(after.status, "unconfirmed"); assert.equal(after.errorCode, "rsvp-source-changed"); assert.equal(fixture.state.posts, 1); continue; }
        if (scenario.startsWith("live-accept") || scenario === "live-tentative") {
          ...
          console.log(`Graph RSVP ${recurring ? "occurrence" : "one-off"} DB ${scenario}: OK`); continue;
        }
        if (scenario === "decline-pull-before-ack") {
        ...
        console.log(`Graph RSVP ${recurring ? "occurrence" : "one-off"} DB ${scenario}: OK`);
