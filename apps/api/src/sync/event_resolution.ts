...
async function prepare(
  ...
  if (row.payload.caldavSeries) {
    ...
    const patch = restoringExdates ? { recurrence: savedWrite.patch.recurrence } : zoneConversion || savedWrite.cancelTarget || savedWrite.followingDelete ? {} : { title: localTarget.title, description: localTarget.description ?? null, location: localTarget.location ?? null, ...(savedWrite.patch.recurrence !== undefined ? { recurrence: savedWrite.patch.recurrence } : {}) };
    const prepared = {
  ...
  if (
    ...
    (remote && !(row.provider === "microsoft" ? microsoftEventVersion(remote.ref.etag) : strongEventEtag(remote.ref.etag))) ||
    (action === "delete" && !ref) ||
