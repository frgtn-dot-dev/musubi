...
export async function prepareCaldavSeries(context: CaldavSeriesContext, input: unknown): Promise<CaldavSeriesPrepared> {
  ...
  if (!["series", "occurrence"].includes(request.scope) || request.action !== "update" || request.time !== undefined || Object.keys(request.patch).some(key => !["title", "description", "location"].includes(key)))
  ...
  try {
    ...
    return { context, write: prepareCaldavSeriesWrite(evidence, baseline, request.patch, target?.id) };
  } catch (error) {
