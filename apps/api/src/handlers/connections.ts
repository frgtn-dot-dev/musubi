...
}).strict().refine((input) => !input.accountId || !!input.provider);
...
export async function handlerSyncConnections(req: Request, res: Response) {
  ...
  if (!parsed.success) throw new BadRequestError("Invalid provider/account sync request");
  try {
    await syncUser(req.user!.id, { ...parsed.data, throwOnError: true });
  } catch {
    throw new BadRequestError("Connected, but the calendar sync failed. Please retry.");
  }
  res.sendStatus(200);
}
