...
export async function handlerGetProviderEventState(req: Request, res: Response) {
  ...
  res.json(await observeProviderOrganizer(req.user!.id, id, await observeMicrosoftRsvp(req.user!.id, id, await observeCaldavRsvp(req.user!.id, id, await caldavAlarmObservation(req.user!.id, id, observation))), req.query.outlookOrganizer === "5" ? "occurrence-time" : req.query.outlookOrganizer === "4" ? "series-content" : req.query.outlookOrganizer === "3" ? "content" : req.query.outlookOrganizer === "2" ? "series" : req.query.outlookOrganizer === "1"));
}
