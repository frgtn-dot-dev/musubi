...
import type { useCalendarTasks } from "@/hooks/useCalendarTasks";
...
import { eventDay, isCalendarTask } from "@musubi/calendar";
...
export default function AgendaTab({ calendarTasks }: { calendarTasks: ReturnType<typeof useCalendarTasks> }) {
  const api = useApi();
  ...
  const groups = useMemo(() => {
    const now = new Date();
    const sorted = [...expansion.events, ...calendarTasks.items]
      .filter(
        (e) =>
          (e.isAllDay
    ...
    return result;
  }, [expansion.events, calendarTasks.items, activeCals, currentDay]);
            ...
            >
              ...
              <View style={{ flexDirection: "row", flex: 4 }}>
                ...
                <View style={{ paddingLeft: 16, justifyContent: "center" }}>
                  <Text style={styles.timelineTitle}>{isCalendarTask(e) ? <><TaskStatusIcon status={e.calendarTask.status} size={14} color={colors.fg} />{" "}<Text style={{ textDecorationLine: e.calendarTask.status === "completed" ? "line-through" : "none" }}>{e.title}</Text></> : e.title}</Text>
                  <View
                    style={{
                      flexDirection: "row",
                      alignItems: "center",
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={[styles.header, { zIndex: 30, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]}>
        <ModeSwitch mode="agenda" onAgenda={() => {}} onChange={view => router.setParams({ view })}
          trigger={open => <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}><Text style={styles.screenTitle}>Agenda</Text><Feather name={open ? "chevron-up" : "chevron-down"} size={16} color={colors.fg3} /></View>} />
