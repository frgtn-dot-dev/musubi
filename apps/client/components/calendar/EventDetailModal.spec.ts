...
const mocks = vi.hoisted(() => ({ request: vi.fn(), close: vi.fn(), toast: vi.fn(), choose: vi.fn(), confirm: vi.fn(), reminders: vi.fn() }));
vi.mock("react", async (original) => ({
  ...await original<typeof import("react")>(),
  useEffect: () => {}, useState: (value: unknown) => [value, vi.fn()],
}));
vi.mock("react-native", () => ({ Text: "Text", Pressable: "Pressable", ScrollView: "ScrollView", View: "View", Linking: {}, Platform: { OS: "android" } }));
...
vi.mock("@/lib/confirm", () => ({ chooseOption: mocks.choose, confirm: mocks.confirm }));
vi.mock("@/store/useCalendarsStore", () => ({ useCalendarsStore: () => ({ calendars: [{ id: "calendar", creatorID: "owner", role: "owner", name: "Calendar" }] }) }));
vi.mock("@/store/useSettingsStore", () => ({ useSettingsStore: () => ({ timeFormat: "24h", dateFormat: "dmy" }) }));
vi.mock("@/store/useAttendeesStore", () => ({ useAttendeesStore: () => ({ byEvent: {}, setAttendees: vi.fn() }) }));
vi.mock("@/services/eventsCache", () => ({ cacheDeleteEvents: vi.fn(), cacheUpsertEvents: vi.fn() }));
vi.mock("@/services/federation", () => ({ setHomeRequester: vi.fn(), remoteForCalendar: vi.fn(), fedFetch: vi.fn() }));
vi.mock("@/services/notifications", () => ({ setReminderWriter: vi.fn(), cancelEventNotification: vi.fn(async () => {}), syncScheduledReminders: mocks.reminders }));
...
const master = EventSchema.parse({
  id: "event", creatorID: "owner", organizer: "owner", title: "Keep detail", color: "#7A8BA3",
  start: "2026-07-06T09:00:00Z", end: "2026-07-06T10:00:00Z", isAllDay: false, isCanceled: false,
  calendars: ["calendar"], originCalendarID: "calendar", recurrence: "RRULE:FREQ=WEEKLY",
});
...
function deleteButton(node: ReactNode): (() => void) | undefined {
  ...
  if (!isValidElement<{ onPress?: () => void; children?: ReactNode }>(node)) return;
...
it.each(["denied", "unknown", "unsupported"])("native scope callback serializes all steps but never persists intent after %s", async (reason) => {
  const api = useApi();
  const message = `This recurrence operation is ${reason}. No changes were saved.`;
  mocks.choose.mockImplementation((_title, _message, options) => options[0].onPress());
  ...
  await expect(applySeriesEdit({
    edited, master, occurrence,
    addEvent: (event) => useEventsStore.getState().addEvent(event, api),
    updateEvent: (event) => useEventsStore.getState().updateEvent(event, api),
...
for (const scope of ["This event only", "This and following events", "All events", "plain"]) {
  it.each(["denied", "unknown", "unsupported", "success"])(`real native ${scope} callback awaits save and preserves detail on %s`, async (reason) => {
    let complete!: (value: unknown) => void;
    mocks.request.mockImplementationOnce(() => new Promise((resolve) => { complete = resolve; }));
    const button = deleteButton(EventDetailModal({ visible: true, event: scope === "plain" ? { ...occurrence, recurrence: null } : occurrence, onClose: mocks.close, onEdit: vi.fn() }));
    expect(button).toBeDefined();
    button!();
    const callback = scope === "plain" ? mocks.confirm.mock.lastCall![1]
      : mocks.choose.mock.lastCall![2].find((option: { label: string }) => option.label === scope).onPress;
    ...
    if (reason === "success") {
      ...
      expect(mocks.reminders).toHaveBeenCalledTimes(deletion ? 0 : 1);
    } else {
      expect(mocks.close).not.toHaveBeenCalled();
      expect(mocks.reminders).not.toHaveBeenCalled();
