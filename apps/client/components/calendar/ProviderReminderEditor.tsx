...
import type { Event, ProviderEventStateResponse, ProviderReminderEdit } from "@musubi/types";
        ...
        {notice ? <Text accessibilityLiveRegion="polite" style={copy}>{notice}</Text> : <>
          ...
          {draft.mode === "custom" ? <>
            {draft.overrides.map((item, index) => <View key={index} style={styles.fieldContainer}>
              ...
              <Btn variant="secondary" label={`Reminder ${index + 1} method: ${item.method === "popup" ? "Notification" : "Email"}`} disabled={busy} onPress={() => setPicker(index)} />
            ...
            <Btn variant="secondary" label="Add reminder" disabled={busy || draft.overrides.length >= 5} onPress={() => setDraft(current => ({ ...current, overrides: [...current.overrides, { method: "popup", minutes: "15" }] }))} />
          </> : null}
          {error ? <Text accessibilityRole="alert" style={copy}>{error}</Text> : null}
          <Btn label="Save Google reminders" loading={busy} onPress={() => void save()} />
        </>}
        <Btn label={notice ? "Close Google reminders" : "Cancel Google reminders"} variant="secondary" disabled={busy} onPress={close} />
      </ScrollView>
    </View>
    </KeyboardAvoidingView>
    <OptionPicker visible={picker !== null} title={picker === "mode" ? "Reminder mode" : "Reminder method"} value={picker === "mode" ? draft.mode : typeof picker === "number" ? draft.overrides[picker]?.method : undefined} options={picker === "mode" ? [{ value: "defaults", label: "Calendar defaults" }, { value: "off", label: "Off" }, { value: "custom", label: "Custom" }] : [{ value: "popup", label: "Notification" }, { value: "email", label: "Email" }]} onClose={() => setPicker(null)} onSelect={value => {
