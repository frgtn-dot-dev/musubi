...
export default function EventDetailModal({
	...
	useEffect(() => {
		if (visible && event?.hasAttendees)
			api
				.getEventAttendees(event)
				.then((a) => setAttendees(event.id!, a))
				.catch(() => {});
	...
	const mine = userID
		? attendees?.find((a) => a.id === userID)?.status
		: undefined;
	...
	const answer = async (tapped: Attendee["status"]) => {
		...
		try {
			...
			showToast({
				message: userFacingError(error, "Your answer could not be saved."),
			});
		}
	};
	...
	const canEditContent = event?.originCalendarID
		...
		: !!event &&
			event.calendars.some((id) =>
				can(calendars.find((c) => c.id === id)?.role, "editEvents"),
			);
	...
	const { timeFormat, dateFormat } = useSettingsStore();
	...
	const accent =
		(originCal ?? calendars.find((c) => event?.calendars.includes(c.id)))
			?.color ?? colors.fg3;
						...
						>
							...
							<View style={{ flex: 1, gap: 6 }}>
								...
								>
									...
									{!event || sameDay
										? ""
										: " – " + formatDateLong(event.end, dateFormat)}
								</Text>
								{!event?.isAllDay && (
									<Text
										style={{
                  ...
                  bottom border when nothing follows (avoids an empty "section"). */}
							...
							{event?.hasAttendees && attendees && (
								...
								<View
									style={[
										styles.fieldContainer,
										{ borderBottomWidth: 0, paddingHorizontal: 26 },
									]}
								>
									<View
										style={{
														...
														>
															<Text
																style={{
																	fontFamily: fonts.sans,
																	fontSize: 10,
																	color: chosen ? colors.onFill : colors.fg2,
																}}
															>
																{choice.label}
															</Text>
                ...
                verbs sit in the middle, Delete last. */}
						...
						<CalendarPickerModal
							...
							onSelect={async (calendarID) => {
								if (event) await linkEvent(event, calendarID, api);
							}}
						/>
						<CalendarPickerModal
							...
							onSelect={async (calendarID) => {
								if (event) await forkEvent(event, calendarID, api);
							}}
						/>
						<CalendarPickerModal
							...
							onSelect={async (calendarID) => {
								if (event) await removeEvent(event, api, calendarID);
							}}
						/>
					</Animated.View>
				</GestureDetector>
