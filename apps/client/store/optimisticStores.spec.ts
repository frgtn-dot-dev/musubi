...
vi.mock("@/hooks/useRefreshData", () => ({
  refreshEventData: vi.fn().mockResolvedValue(undefined),
}));
...
describe("optimistic stores", () => {
  it("updates an event immediately and rolls back on failure", async () => {
    ...
    const pending = useEventsStore
      .getState()
      .updateEvent({ ...event, title: "After" }, {
        updateEvent: () => request,
      } as any);
  ...
  it("removes a calendar immediately and rolls back on failure", async () => {
    ...
    const pending = useCalendarsStore
      .getState()
      .removeCalendar(calendar, { removeCalendar: () => request } as any);
...
it("never rolls back a committed failure without a readable current row", async () => {
  ...
  for (const action of ["updateEvent", "addEvent"] as const) {
    ...
    await expect(
      useEventsStore.getState()[action]({ ...event, title: "Committed" }, {
        updateEvent: async () => {
          throw error;
        },
        createEvent: async () => {
...
for (const removal of ["revision", "full", "calendar"] as const) {
  for (const failure of [false, true])
    it(`does not resurrect on delayed ${failure ? "committed error" : "success"} after ${removal} removal`, async () => {
      const { EventMutationError } = await import("@musubi/types");
      const { cacheUpsertEvents } = await import("@/services/eventsCache");
      const { syncScheduledReminders } = await import(
      ...
      if (removal === "revision")
        await useEventsStore
          .getState()
          .localRemoveEvent({ ...event, revision: 3 });
      ...
      if (failure) {
        reject(
          new EventMutationError("Saved locally", true, {
            ...event,
            revision: 2,
          }),
        );
...
for (const reconciliation of [
  ...
  for (const action of [
    ...
    it(`applies B ${action} including cache/reminders after ${reconciliation} of A`, async () => {
      ...
      const { cacheDeleteEvents, cacheUpsertEvents } = await import(
        "@/services/eventsCache"
      );
      const { cancelEventNotification, syncScheduledReminders } = await import(
        "@/services/notifications"
      );
      ...
      const pending =
        action === "linkEvent" || action === "forkEvent"
      ...
      if (reconciliation === "other-removal")
        ...
        useEventsStore
          .getState()
          .loadEvents(
            useEventsStore.getState().events.filter((e) => e.id !== event.id),
          );
      vi.clearAllMocks();
      const result = {
        ...b,
        id:
          reconciliation === "other-removal" &&
          (action === "forkEvent" || action === "addEvent")
            ? "server-created"
            : b.id,
        title: "Confirmed",
        revision: 2,
      ...
      finish(
        action === "removeEvent"
          ? { id: b.id, removed: true, calendars: [], revision: 2 }
          : result,
      );
      await pending;
      if (action === "removeEvent") {
        ...
        expect(syncScheduledReminders).toHaveBeenCalledWith([result], {
          onlyEventIDs: [result.id],
        });
        if (action === "addEvent" && result.id !== b.id)
          expect(
            useEventsStore.getState().events.some((e) => e.id === b.id),
          ).toBe(false);
      }
    });
  }
...
it("source removal cannot fence an independent fork, while a known newer removal of the returned ID does", async () => {
  const { EventSchema } = await import("@musubi/types");
  const source = EventSchema.parse({
    ...event,
    creatorID: "owner",
    organizer: "owner",
    color: "red",
  ...
  for (const removedID of [source.id, "fork"]) {
    ...
    let finish!: (value: any) => void;
    const pending = useEventsStore.getState().forkEvent(source, "c2", {
      forkEvent: () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
    ...
    await useEventsStore
      .getState()
      .localRemoveEvent({ ...source, id: removedID, revision: 3 });
    finish({ ...source, id: "fork", revision: 2 });
    if (removedID === "fork") {
      await expect(pending).rejects.toThrow("Saved locally");
      expect(
        useEventsStore.getState().events.some((e) => e.id === "fork"),
      ).toBe(false);
    } else {
      await pending;
      expect(
        useEventsStore.getState().events.find((e) => e.id === "fork")?.revision,
      ).toBe(2);
    }
  }
});
...
for (const outcome of ["success", "current-error"] as const) {
  for (const receiptRevision of [2, 4]) {
    it(`${outcome} revision ${receiptRevision} is ordered against target removal 3; fresh authoritative revival still works`, async () => {
      ...
      const target = EventSchema.parse({
        ...event,
        creatorID: "owner",
        organizer: "owner",
        color: "red",
      ...
      const pending = useEventsStore.getState().updateEvent(target, {
        updateEvent: () =>
      ...
      await useEventsStore
        .getState()
        .localRemoveEvent({ ...target, revision: 3 });
      if (outcome === "success")
        finish({ ...target, revision: receiptRevision });
      else
        fail(
          new EventMutationError("Saved locally", true, {
            ...target,
      ...
      if (outcome === "current-error" || receiptRevision < 3)
        await expect(pending).rejects.toThrow("Saved locally");
      else await pending;
      expect(useEventsStore.getState().events.length).toBe(
        receiptRevision > 3 ? 1 : 0,
      );
      await useEventsStore
        .getState()
        .localUpdateEvent({ ...target, revision: 5 });
      await useEventsStore
        .getState()
        .localRemoveEvent({ ...target, revision: 4 });
...
it("account/server reset fences even a server-assigned created identity", async () => {
  let finish!: (value: any) => void;
  const pending = useEventsStore.getState().addEvent(event, {
    createEvent: () =>
      new Promise((resolve) => {
        finish = resolve;
      }),
