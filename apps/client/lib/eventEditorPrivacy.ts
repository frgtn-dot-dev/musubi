...
export function privateEditorRefresh(snapshot: Event | undefined, current: Event | undefined, calendars: Calendar[], removed = false): Event | undefined {
  ...
  if (source?.provider !== "google" || !(observed?.sourceID === source.id || source.role === "viewer" || (
    current.title === "Busy" && !current.description && !current.location && !current.url && !current.organizer
  ))) return;
  const refreshed = { ...snapshot, title: current.title, description: current.description, location: current.location, url: current.url, organizer: current.organizer, color: current.color };
