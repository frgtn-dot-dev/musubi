...
it("routes the same atomic time draft through home and federated transports", async () => {
  ...
  const { editEventTimeDraft, knownEventTimeDraft, resolveEventTimeEdit } = await import("@musubi/calendar");
  ...
  const edited = editEventTimeDraft(event, { ...event, title: "Together" }, { ...knownEventTimeDraft(event)!, date: "2026-03-30", endDate: "2026-03-30" });
