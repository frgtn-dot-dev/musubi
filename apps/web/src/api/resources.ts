...
export function getProviderEventState(eventId: string, signal?: AbortSignal, connectionId?: string) {
  return apiRequest(route(connectionId, `/api/v1/events/${encodeURIComponent(eventId)}/provider-state${connectionId ? "" : "?outlookOrganizer=10&outlookRsvp=1"}`), { responseSchema: ProviderEventStateResponseSchema, signal });
}
