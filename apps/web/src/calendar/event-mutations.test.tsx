...
vi.mock("./federated-workspace", () => ({
  useFederatedWorkspace: () => ({ data: { calendars: [] } }),
}));
  ...
  for (const removal of ["tombstone", "access-loss"] as const) {
    for (const outcome of [
      ...
      it(`does not resurrect ${removal} after delayed ${outcome}`, async () => {
        ...
        const { result } = renderHook(() => useEventMutations("user"), {
          wrapper,
        });
        let pending!: Promise<unknown>;
        await act(async () => {
          pending = result.current.updateEvent(event);
        });
        client.setQueryData(key, {
          events: [],
          deletedIds: removal === "tombstone" ? [event.id] : [],
          serverTime: "newer",
        });
        await act(async () => {
          if (outcome === "success") {
            resolve({ ...event, revision: 2 });
            await pending;
          } else {
            reject(
...
for (const outcome of ["success", "current-error"] as const) {
  for (const removal of ["tombstone", "access-loss"] as const) {
    it(`keeps newly fetched date/view query absent on old ${outcome} (${removal}) while updating unchanged Q1`, async () => {
      ...
      const key = (date: string, view: string) =>
      ...
      const unsubscribe = client
        .getQueryCache()
        .subscribe(() => observed.push(client.getQueryData(q2)));
      await act(async () => {
        if (outcome === "success") {
          resolve({ ...event, revision: 2 });
          await pending;
      ...
      expect(observed.every((value) => value === client.getQueryData(q2))).toBe(
        true,
      );
      expect(client.getQueryData(q1)).toMatchObject({
        events: [{ revision: 2 }],
      });
      unsubscribe();
      client.clear();
