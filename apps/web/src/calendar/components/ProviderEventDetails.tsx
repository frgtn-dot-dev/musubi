...
function ProviderEventDetailsBody({ providerFlavor, presentation = "default", event: sourceEvent, seriesMaster, eventId, userId, connectionId, series = false, occurrence = false, onEditReminders, onRespond, moveTrigger, onMove }: Props & { moveTrigger: React.RefObject<HTMLButtonElement | null>; onMove: () => void }) {
  ...
  const actions = <>
    ...
    {current?.rsvpEdit && current.state && current.version && !series ? <Button variant="secondary" loading={opening} onClick={event => void openEditor(event.currentTarget, "rsvp")}>{occurrence || current.rsvpEdit.scope === "occurrence" ? "Respond to this occurrence" : current.rsvpEdit.provider === "microsoft" ? "Respond in Outlook" : current.rsvpEdit.provider === "caldav" ? "Respond in calendar" : "Respond in Google"}</Button> : null}
